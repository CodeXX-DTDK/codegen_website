...
<Alert type="caution">
**🚧 Roadmap — WIP.** The `find_package(codegen)`, `codegen_rule()`, and `CODEGEN_OUTPUTS` helpers below are the design target for the [Team-tier](/licensing/tiers/) `cmake_integration` feature and are **not yet shipped**. The supported path today is `add_custom_command` driving the `codegen` binary, with `--use-workspace` reading includes/defines from `codexx.workspace.yaml` (or `compile_commands.json`). This page documents both the current and the planned path; track progress in the codegen changelog.
</Alert>
...
If you use `--use-workspace`, run codegen from the project root so the build-system config is found. Pass `--workspace-dir` to point at a different directory.
