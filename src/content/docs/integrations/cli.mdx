...
| `--use-workspace` | Auto-load includes and defines from a build-system config (`codexx.workspace.yaml` or CMake file-api). Default workspace root is the CWD. |
...
| Flag | Default | Description |
|---|---|---|
| `--dry-run` | off | Don't write outputs. With a Professional+ licence, opens the TUI diff viewer; otherwise emits `E102` and prints the count of files that would be affected. |
...
# Dry-run preview (TUI diff viewer with a Professional licence)
