...
Activation contacts the licence service once, verifies the response signature, and stores a signed token locally. No subsequent internet access is required while the cache TTL holds. See [License Activation](/licensing/activation/) for the cached-licence model and the air-gapped path.
