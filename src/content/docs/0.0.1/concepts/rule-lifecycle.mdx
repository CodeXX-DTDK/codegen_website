...
description: The sequence of script invocations for a single rule run, in the
  order the engine actually executes them.
slug: 0.0.1/concepts/rule-lifecycle
---
...
Files are written via a temp-file-and-rename pattern. Under `--dry-run`, contents are collected for the TUI diff viewer instead (Professional+; the gate emits `E102` otherwise).

## Phase 11: Inline edits

For each entity that returned `inline` items, the engine rewrites the source header at the matching anchor (bare or paired form). Bare anchors are expanded to the paired form on first run. See [Inline Injection](/0.0.1/rules/inline-injection/).
...
| License `E102` (e.g. dry-run TUI on Community) | Exit 0 with the count of would-be files; no diff shown. |
