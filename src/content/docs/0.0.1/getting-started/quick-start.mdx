...
slug: 0.0.1/getting-started/quick-start
---
...
<Steps>
  1. **Set up the rule directory**
     ...
     The four files in a rule directory are documented in [Rule Anatomy](/0.0.1/rules/anatomy/).
  ...
  5. **Add the generated file to your build**
     ...
     To re-trigger codegen automatically when the header changes, drive the binary from `add_custom_command` — see [CMake Integration](/0.0.1/integrations/cmake/) for the current pattern (the packaged `find_package(codegen)` flow is on the roadmap).
</Steps>
...
Read [Your First Rule](/0.0.1/getting-started/first-rule/) to write a rule from scratch.

<Alert type="info" title="Working with an LLM agent?">
  The full docs are bundled as a single plain-text file at [`/llms-full.txt`](/0.0.1/llms-full.txt).
  ...
  crawling required. A condensed index is also available at [`/llms-small.txt`](/0.0.1/llms-small.txt).
</Alert>
...
<Alert type="info" title="Key Takeaways">
  ...
  * Attribute annotations (`[[codegen::Rule]]`) are the alternative trigger for struct/class-level rules.
