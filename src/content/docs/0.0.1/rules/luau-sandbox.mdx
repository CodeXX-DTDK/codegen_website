---
title: LuaU Sandbox
description: What the LuaU VM provides to rule scripts, what it withholds, and
  how to reason about rule safety.
slug: 0.0.1/rules/luau-sandbox
---
...
| Native introspection API | `get_node`, `find_struct_by_name`, `struct_has_attribute`, `struct_has_method`, `struct_has_member`, `struct_has_default_constructor`, `get_base_classes`, `get_definition_header` — see [LuaU Globals](/0.0.1/reference/luau-globals/) |
...
Or drop a `.env` next to the rule, or a shared `<rulesDir>/../shared/.env`. Either path activates the env capability and exposes the values via the env module. See [Permissions](/0.0.1/rules/permissions/) for surface details.
