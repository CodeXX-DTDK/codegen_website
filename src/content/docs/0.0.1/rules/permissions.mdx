...
description: How to grant rules access to HTTP, OS environment variables, and
  dotenv values, and why the default is to grant nothing.
slug: 0.0.1/rules/permissions
---
...
Every rule runs with no I/O capabilities by default. The LuaU sandbox provides `json`, the native introspection API, and a stdlib subset (see [LuaU Globals](/0.0.1/reference/luau-globals/)). No filesystem, no network, no env, no process execution.
