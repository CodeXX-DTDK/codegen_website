---
title: Rule Anatomy
description: The files that make up a codegen rule, config, preamble, grouping,
  and handler, and the contract each must satisfy.
slug: 0.0.1/rules/anatomy
---
...
A workspace-shared directory at `<rulesDir>/../shared/` (e.g. `.codegen/shared/`) is loaded automatically: every `.luau` file there is concatenated in alphabetical filename order and prepended to each rule's transform at execution time, and a `shared/.env` file becomes the baseline dotenv. By default every shared script is included; rules can narrow the set with `shared.include` / `shared.exclude` globs in their config (see [Config Schema Reference](/0.0.1/reference/config-schema/#shared)). This is the [Team-tier](/0.0.1/licensing/tiers/) shared-libraries feature; today the directory is loaded unconditionally and the licence gate ships in a future release.
...
Full schema: see [Config Schema Reference](/0.0.1/reference/config-schema/).
...
The third segment of the anchor is the *qualified name* of the target entity (namespace path included). Anchors are scanned per-file and matched against the AST to find the corresponding entity. See [Inline Injection](/0.0.1/rules/inline-injection/) for the on-disk anchor lifecycle.
