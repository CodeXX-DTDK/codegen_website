...
slug: 0.0.1/reference/diagnostics
---
...
| `E101` | License invalid key (no signed token, malformed activation key) | Run `codegen license activate <key>`. |
| `E102` | Feature requires a higher tier (e.g. `--dry-run` TUI viewer is Professional+) | See [Tiers & Pricing](/0.0.1/licensing/tiers/). |
| `E103` | Cached license token failed signature, machine-mismatch, expired, suspended, or corrupt | Re-activate with `codegen license activate <key>`. |
