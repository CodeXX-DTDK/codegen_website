...
slug: 0.0.1/reference/luau-globals
---
...
The URL's host component is matched against the allowlist (exact host, `*.domain` wildcard, or `*` as escape hatch — see [`permissions.http.allowlist`](/0.0.1/reference/config-schema/#permissionshttpallowlist)). A URL whose host is not on the allowlist raises a runtime error and emits diagnostic `E010`. Concurrent requests on the same VM are not supported.
...
The env module exposes the allowlisted OS env vars and the merged dotenv (CLI `--env` > per-rule `.env` > shared `.env`). See the [Permissions](/0.0.1/rules/permissions/) page for the surface API.
...
* `debug` (Lua debug library; the DAP debugger is a separate feature, see [VS Code DAP Debugger](/0.0.1/integrations/vscode-debugger/))
